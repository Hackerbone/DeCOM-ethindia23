...
const OnboardingStep4 = () => {
    ...
    return (
        <div className={styles.onboardingStepContainer}>
            ...
            <p className={styles.description}>Your store is now deployed and can be accessed at
                {" "}<a href={`http://127.0.0.1:3000/${storeId}`} target="_blank" rel="noreferrer">http://127.0.0.1:3000/${storeId}</a></p>
            <Form className={`${formStyles.formContainer} ${styles.onboardingForm}`} layout='vertical'>
                <Form.Item
                    className={formStyles.formItem}
                    style={{display: "flex", justifyContent: "center"}}>
                    <Image preview={false} src={checkMark} width={150} height={150} className={styles.checkMark} />
                </Form.Item>
                <PrimaryButton
